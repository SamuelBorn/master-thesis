...
transforms the graph into a planar form by eliminating crossings.


\todo{There exist other algorihtms like bentley ottman or even linear time but it was easier to implement this one. not performance critical.}
For efficient execution, we employ a spatial index that stores the bounding boxes of all
edges. Under the assumptions of short edges this structure enables rapid identification of potential intersections
...
\begin{figure}
	...
	\begin{subfigure}{0.45\linewidth}
		...
		\begin{tikzpicture}[every node/.style={circle, draw, minimum size=1cm, node distance=1.5cm}]
			...
			\draw[dashed] (-1, 1) -- (-1, -4.5);
		\end{tikzpicture}
        \caption{If nodes 1 and 3 were forced to be part of the separator (because of other reasons), the separator of the planarized graph would be larger.}
	\end{subfigure}
