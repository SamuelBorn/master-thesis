...
This approach reflects the real-world phenomenon where new developments, even smaller ones, can connect to or emerge around established, major infrastructure hubs that might have formed at much earlier stages of development; for instance, a small town developing near a major motorway interchange.
...
This approach reflects the real-world phenomenon where new settlements, even smaller ones, can emerge around established, major infrastructure hubs that.
...
This global Delaunay triangulation typically results in a graph with an average vertex degree of 6, which is denser than typical road networks.
This again can see by looking at Euler's formula for planar graphs, \(V - E + F = 2\).
Since every face (but the outer face) of the graph is a triangle and each edge is shared by two faces, we have approximately \(3F \approx 2E\).
If we use this approximation to eliminate \(F\) from the equation and solve for \(2E / V\) (the average degree).
...
\begin{algorithm}[tbhp]
	...
	\BlankLine
	\ForAll{\((u, v) \in E\) sorted by length}{
		\If{\(dist(u, v) > \alpha \cdot len(u, v)\)}{
...
Drawing from observations of real road networks and prior work on synthetic graph generation \cite{hutchison_synthetic_2010}, \(L\) is often set to 4, which has been shown suitable for generating graphs with up to approximately 20 million nodes.
With \(L=4\), the algorithm requires parameters for level expansion fractions \(f_i\), points per expansion site \(k_i\), and expansion radii \(r_i\), for \(i \in [1, L]\), totaling \(3L = 12\) parameters.
However, the effective number of free parameters is lower.
The specific value of the first expansion radius, \(r_1\), primarily scales the entire embedding without altering its topological structure, effectively reducing the count to 11 if \(r_1\) is fixed or normalized.
Furthermore, the algorithm specifies that the first level expansion fraction \(f_1 = 1.0\) (ensuring the initial point always seeds the first level of expansion), which reduces the free parameters to 10.
If we aim for a target total number of vertices, \(n_{\text{target}}\), one additional parameter (e.g., \(k_L\)) can be adjusted to meet this target, potentially leaving 9 free parameters for a 4-layer model.

The total number of points after level \(j\), denoted \(n_j\) (where \(n_1\) represents the points after the first expansion around the initial seed point), is given by the recurrence relation:
\[
	n_j =
	\begin{cases}
		k_1 + 1                               & \text{if } j=1                        \\
...
The median edge length in a Delaunay triangulation of \(n\) points within a region of effective radius \(r\) is heuristically proportional to \bigO{r\cdot n^{-1/2}}.
Thus, to maintain somewhat consistent local geometric scales as density increases within nested expansions (if \(k_i\) is relatively stable), \(r_i\) needs to decrease.
Our empirical observations suggested that without exponential decrease in radii across levels, achieving consistently small separators was not possible.
...
Consider a thought experiment: if each expansion site has an infinitesimally small influence radius for generating points for next level, these new points primarily connect among themselves.
The number of points on the effective \enquote{perimeter} of such an expanded site (those able to connect to other, distant parts of the graph) may grow much slower than the number of points generated internally within the site thus keeping its external connectivity limited (this is exaggerated by by the hierachical sampling).
In such a scenario, one can find a separator for the graph defined by the top-level points (e.g., \(n_1\) points for the first level) with a size scaling as \bigO{n_1^{1/2}}.
If each of these top-level separator nodes is a gateway to a large hierarchical substructure that is only sparsely connected back to this top-level framework (via the few \enquote{perimeter} connections), the separator for the entire graph \(G_L\) can be estimated based on this top-level cut extended by these few perimeter vertices that are not part of the top-level separator.
Given that the total number of nodes \(n_L\) is typically much larger than \(n_1\), such a separator (e.g., related to a \bigO{n_1^{1/2}} scaling) would likely be disproportionately small for \(n_L\).
This issue can also exists on subsequent levels.
An attempt to achieve a desired global scaling \bigO{n_L^{1/3}} can involve setting parameters so that the initial \bigO{n_1^{1/2}} separator scaling corresponds to this overall target.
This, however, can lead to a different problem: distinct top-level expansion sites, even if spatially close, end up connected by very few paths.
...
\begin{figure}[tbhp]
	...
	\caption{Separator size scaling for a hierarchical Delaunay graph generated with a constant number of expansion sites parameters: \(f_i=(1.0, 0.3, 0.2, 0.1)\); \(k_i=(50, 50, 50, 50)\); \(r_i=(1000, 424, 120, 17)\).}
...
Such high local density means that partitioning these individual dense regions themselves requires larger separators, reflecting their increased internal complexity (if these dense regions were considered as isolated subgraphs, their own separator sizes would be expected to be larger).
Simultaneously, by populating the same expansion radius \(r_i\) with more points, the average distances between points decrease on this level.
...
Note that radii on levels deeper in the hierachy have to be changed too, as otherwise a larger radius at the higher level would decrease the average distance between points in the next level, which would lead to a smaller separator size.
...
For instance, to simulate prominent \enquote{urban centers} at higher levels, a larger radius \(r_2\) might be employed.
...
Subsequently, \cref{fig:param_interplay_fix1_radii} shows that this reduction in separator size due to a low \(f_2\) can be counteracted by proportionally increasing the expansion radius of the second level (\(r_2\)) and the subsequent radii (\(r_3, r_4\)) to match the same ratio.
...
\begin{figure}[tbhp]
	...
	\begin{subfigure}{0.45\linewidth}
		\centering
		\includegraphics[width=\linewidth]{graphics/interplay_low_f1.pdf}
