...
If the scale values are chosen as powers of 2 adding \(3 \text{scale}\) to the coordinates ensures that the noise samples at different scales do not overlap.
...
To transform this continuous output into a binary map of allowed versus disallowed regions, we apply a noise threshold (e.g., \(0.5^M\)): a point is considered in an \enquote{allowed} region if its final product value is greater than \(T_{\text{noise}}\); otherwise, it is \enquote{disallowed}.
