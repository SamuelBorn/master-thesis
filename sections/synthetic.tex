...
A nested grid construction with \(\ell\) layers is determined by \(\ell\) grid sizes (specifying the dimensions of the grid at each layer) and \(\ell-1\) placement parameters (specifying how many subgraphs from layer \(i+1\) are placed within layer \(i\)).
...
For our experiments that yield good results for around 10 million nodes, we use a sequence of eight scales (\(M=8\)): \([8, 16, 32, 64, 128, 256, 512, 1024]\). The initial scale of \(8\) is chosen instead of \(1\) because the Perlin noise implementation available to us exhibits overly smooth behavior at scale \(1\) for our purposes.
...
For instance, using the aforementioned eight scales to generate a small graph of only \(10^4\) points results in separators that scale approximately as \bigO{n^{1/2}}.
