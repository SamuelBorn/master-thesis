...
For each point \((x,y)\), Perlin noise is first sampled at \(M\) different scales (often referred to as octaves), where the frequency typically doubles at each successive scale.
...
To transform this continuous output into a binary map of allowed versus disallowed regions, we apply a threshold \(T_{\text{noise}}\) (e.g., \(0.5^M\)): a point is considered in an \enquote{allowed} region if its final product value is greater than \(T_{\text{noise}}\); otherwise, it is \enquote{disallowed}.
...
Essentially, candidate points are randomly sampled within a predefined domain (e.g., a unit circle). The noise value, which is the product of \(M\) scaled and normalized Perlin samples, is computed at each candidate point's location. If this value indicates an \enquote{allowed} region (i.e., above \(T_{\text{noise}}\)), the point is accepted and added to the graph's vertex set; otherwise, it is rejected, and a new candidate is sampled. This process continues until the desired number of \(n\) vertices is collected.
For our experiments that yield good results for up to 10 million nodes, we use a sequence of eight scales (\(M=8\)): \([8, 16, 32, 64, 128, 256, 512, 1024]\). The initial scale of \(8.0\) is chosen instead of \(1.0\) because the Perlin noise implementation available to us exhibits overly smooth behavior at the \(1.0\) scale for our purposes.
...
\begin{algorithm}[tbhp]
	...
	\While{\(|P| < n\)}{
		\((x,y) \longleftarrow\) randomPointInUnitCircle()\;
		\BlankLine
		noise \(\longleftarrow \prod_{i=1}^{M} \text{perlinNoise}(x \cdot s_i + 3s_i,~~~y \cdot s_i + 3s_i)\)\;
		\BlankLine
        \If{\(\text{noise} > 0.5^{M}\)}{
			\(P \longleftarrow P \cup \{(x,y)\}\)\;
		}
	}
	\(G \longleftarrow\) delaunay(\(P\))\;
	pruneEdges(\(G\))\;
