...
For the DIMACS Europe road network, after applying a planarization procedure (detailed in a later section \cref{sec:approach:planarity}), we compute a meshedness coefficient \(\alpha \approx 0.1166\).
...
For comparison, the meshedness coefficient of other well-known planar graph classes serves as a useful benchmark.
For a square grid graph, the coefficient approaches \(0.5\) as \(n \to \infty\), while a full Delaunay triangulation, being a maximal planar graph, has a coefficient approaching 1.
In contrast, urban cores within road networks tend to be more densely meshed than continental-scale networks.
For instance, by analyzing the inner city structure of Karlsruhe, we compute a meshedness coefficient of approximately \(0.25\).
This higher value reflects the more grid-like layout typical of urban centers.

A graph with fine granularity, where roads are represented by long chains of degree-2 vertices, can have its meshedness value artificially skewed lower because adding such vertices increases \(n\) without increasing the graph's cycle count (\(m-n\)).
Therefore, to analyze the underlying junction-based topology independent of this modeling artifact, contracting degree-2 vertices can be a good normalization step.
Applying this normalization step to the DIMACS Europe graph, increases its meshedness coefficient only slightly from approximately \(0.1166\) to \(0.1317\).
...
Our empirical analysis of these contracted subgraphs indicates that their diameter scales approximately as \bigO{n^{0.3098}}.
...
\begin{figure}[tbhp]
	...
	\caption{Empirical scaling of hop diameter with the number of vertices \(n\) for nested disscection subgraphs of road networks (with degree-2 nodes contracted). The observed scaling is approximately \bigO{n^{0.3098}}.}
